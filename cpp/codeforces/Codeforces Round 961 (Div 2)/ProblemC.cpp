...
void solve(){
    ...
    while(it!=freq.end()){
        ...
        if(next==freq.end() || abs(next->first-petals)>1){
            ll cost = pairs.first;
            auto &set = pairs.second;

            for (auto i = set.rbegin(); i!=set.rend() ; i.operator++()) {
                if(cost<=k) mx = max(cost,mx);
            }
