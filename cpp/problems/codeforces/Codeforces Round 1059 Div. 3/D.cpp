...
pair<int,int> getPairQuering(int start, int end) {
...
void solve(){
    int n; cin >> n;
    int size = 0;
    pair<int,int> initial = getPairQuering(1,n);
    size = initial.second-initial.first;
    int searchStart = 1;
    int searchEnd = n;
    int left  = -1;
    while (searchStart<=searchEnd) {
        int mid = searchStart+((searchEnd-searchStart)/2);
        pair<int,int> sum = getPairQuering(searchStart,mid);
