...
void solve(){
    ...
    vector<vector<int>> distance = vector<vector<int>>(N+1,vector<int>(N+1,-1));
    pr(visited,distance);
    queue<ii> q;
    q.push(mp(Sx,Sy));
    visited[Sx][Sy]=1;
    ...
    while(!q.empty()){
        ...
        pr(node);
        // 8 Connect Component for knight 
        for (int i = 0; i < 8; ++i) {
            int ti = node.F + dx[i], tj = node.S + dy[i];
            if(ti>=1 && tj>=1 && ti<=N && tj<=N){
                if(!visited[ti][tj]){
                    visited[ti][tj]=1;
                    distance[ti][tj] = distance[node.F][node.S]+1;
                    q.push(mp(ti,tj));
                }
            }
        }
